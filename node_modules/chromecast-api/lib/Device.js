...
Device.prototype.setPlayer = function(callback){
	var self = this;

	// if(self.player) return callback(true);
	...
	console.log('Connecting to host: ' + self.host)
	...
	self.client.connect(self.host, function () {
		...
		self.client.launch(DefaultMediaReceiver, function (err, player) {
			if (err) {
				console.log('------------ CLIENT LAUNCH ERROR: %s', err)
			...
			player.on('status', function (status) {
				if (status) {
					console.log('PlayerState = %s', status.playerState)
...
Device.prototype.play = function (resource, seconds, callback) {
	...
	this.setPlayer(function(status){
		if(status) self._privatePlayMedia(resource, seconds, callback);
		console.log("PLAY SUCCESS: " + status);
	});
}
...
Device.prototype.getStatus = function (callback) {
	...
	if(!self.player) return callback("No Player found (getStatus)");
	self.player.getStatus(function(err, status) {
		if (err) {
			console.log("Error getStatus: %s", err.message)
			return callback(err)
		}
...
Device.prototype.pause = function (callback) {
	...
	if(self.player){
		...
		console.log("COULD NOT PAUSE, LOST PLAYER REF");
	};
...
Device.prototype.unpause = function (callback) {
	...
	if(self.player){ 
		...
		self.getStatus(function(err, status){
			console.log("------- getStatus:", err, status);
		})
		console.log("COULD NOT UNPAUSE, LOST PLAYER REF");
	}
	
}

Device.prototype.setVolume = function (volume, callback) {
	if (!this.client) return callback(new Error('no client'))
...
Device.prototype.stop = function (callback) {
	...
	this.setPlayer(function(status){
		...
		console.log("STOP SUCCESS: " + status);
	});
}
...
Device.prototype.subtitlesOff = function (callback) {
	if (!this.player) return callback(new Error('no player'))
...
Device.prototype.close = function (callback) {
	var self = this
	if (!self.client) return callback(new Error('no client'))
